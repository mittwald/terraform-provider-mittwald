...
- `id` (String) A pattern to match against article IDs using glob-style wildcards. Use `*` to match any sequence of characters, `?` to match a single character, and `[...]` for character classes.
