...
func testAccAssertMySQLDatabaseIsPresent(resourceName string, databaseOut *databasev2.MySqlDatabase, userOut *databasev2.MySqlUser) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		database, err := apiutils.Poll(ctx, apiutils.PollOpts{}, client.GetMysqlDatabase, databaseclientv2.GetMysqlDatabaseRequest{rs.Primary.ID})
		if err != nil {
			return err
		}
		...
		user, err := apiutils.Poll(ctx, apiutils.PollOpts{}, client.GetMysqlUser, databaseclientv2.GetMysqlUserRequest{userID})
		if err != nil {
			return err
		}
